...
public:
        ...
        for(long long  i : nums){
            if (i==INT_MAX)
                return INT_MAX; // if you do n partitions you won't be able to reduce this, so this is maxx sum
            sum+=i;
        }
        if (m==1) // can't partition it then the max value in the whole array will be the max sum.
            return sum;
        long long low=0;
        long high=sum; // "high" ----------the maximum sum that is an possible answer.
        long long  mid;
    ...
    bool isValid(vector<int> nums,int n, int m, int mid){
        ...
        for(int i : nums){
            ...
            if (sum>mid){
                ...
                sum=i;
                if (sum>mid)
                    //at any instance we find that this value is greater than current max sum we are checking for we return false;
                    return false;
            }
            if (assn>m) 
        ...
        return true; 
        // if everything is fine without violating any condition we return true;
